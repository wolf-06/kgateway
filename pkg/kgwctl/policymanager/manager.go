...
Modified to fix a bug because policies have a list of targetRefs, not a single targetRef.
*/
...
type Policy struct {
	...
	// TargetRef is the same as TargetRefs but only applies to 1 target. TargetRef and TargetRefs should never both be populated at the same time.
	// https://gateway-api.sigs.k8s.io/geps/gep-2648/#migration-from-single-to-multiple-targets
...
func ConstructPolicy(u *unstructured.Unstructured, inherited bool) (Policy, error) {
	...
	if err := runtime.DefaultUnstructuredConverter.FromUnstructured(u.UnstructuredContent(), structuredPolicy); err != nil {
		return Policy{}, fmt.Errorf("failed to convert unstructured policy resource to structured: %w", err)
	}
	if structuredPolicy.Spec.TargetRef != nil {
	...
	for _, targetRef := range structuredPolicy.Spec.TargetRefs {
		...
		if ref.Namespace == "" {
			ref.Namespace = result.Unstructured.GetNamespace()
		}
		if targetRef.Namespace != nil {
			ref.Namespace = string(*targetRef.Namespace)
		}

		result.TargetRefs = append(result.TargetRefs, ref)
	}
...
func (p *Policy) IsAttachedTo(objRef common.GKNN) bool {
	...
	for _, targetRef := range p.GetAllTargetRefs() {
...
func (p *Policy) GetAllTargetRefs() []common.GKNN {
	return slices.DeleteFunc(
		append([]common.GKNN{p.TargetRef}, p.TargetRefs...),
		func(thing common.GKNN) bool {
			return thing == (common.GKNN{})
		},
	)
}
