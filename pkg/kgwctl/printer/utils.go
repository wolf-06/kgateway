...
// convertPoliciesToRefsTable converts a slice of policies to a table of policy references.
// If includeTargets is true, the table will include the target kind and name for each target of each policy.
// If includeTargets is false, the table will only include the policy type and name.
// TODO: Display only the relevant target for each policy.
func convertPoliciesToRefsTable(policies []*policymanager.Policy, includeTargets bool) *Table {
	table := &Table{
	...
	for _, policy := range policies {
		...
		isFirstRow := true
		for _, targetRef := range policy.GetAllTargetRefs() {
			...
			table.Rows = append(table.Rows, refRow)
		}
	}
	return table
}
