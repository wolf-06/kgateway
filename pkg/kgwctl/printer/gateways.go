...
func (p *TablePrinter) printGateway(gatewayNode *topology.Node, w io.Writer) error {
	...
	if p.OutputFormat == OutputFormatWide {
		...
		httpRoutesCount := fmt.Sprintf("%d", len(topologykgw.GatewayNode(gatewayNode).HTTPRoutes()))
...
func (p *DescriptionPrinter) printGateway(gatewayNode *topology.Node, w io.Writer) error {
	...
	// DirectResponses
	directResponses := &Table{
		ColumnNames:  []string{"Kind", "Name"},
		UseSeparator: true,
	}
	httpRouteCount, backendsCount := 0, 0
	httpRouteNodes := maps.Values(topologykgw.GatewayNode(gatewayNode).HTTPRoutes())
	for _, httpRouteNode := range topology.SortedNodes(httpRouteNodes) {
		...
		backendNodes := maps.Values(topologykgw.HTTPRouteNode(httpRouteNode).Backends())
	...
	pairs = append(pairs,
		&DescriberKV{Key: "AttachedRoutes", Value: attachedRoutes},
		&DescriberKV{Key: "Backends", Value: backends},
		&DescriberKV{Key: "DirectResponses", Value: directResponses},
	)
