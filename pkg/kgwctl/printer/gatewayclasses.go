...
Modified to display kgateway specific resource relations.
*/
...
func (p *TablePrinter) printGatewayClass(gatewayClassNode *topology.Node, w io.Writer) error {
	...
	if p.OutputFormat == OutputFormatWide {
		gatewayCount := fmt.Sprintf("%d", len(topologykgw.GatewayClassNode(gatewayClassNode).Gateways()))
...
func (p *DescriptionPrinter) printGatewayClass(gatewayClassNode *topology.Node, w io.Writer) error {
	...
	gatewaysCount := 0
	gatewayNodes := maps.Values(topologykgw.GatewayClassNode(gatewayClassNode).Gateways())
