...
type typedPrinter interface {
	...
	printDirectResponse(*topology.Node, io.Writer) error
	printUnknown(*topology.Node, io.Writer) error
}

func parseAndPrint(node *topology.Node, w io.Writer, p typedPrinter) error {
	if node.Metadata != nil && node.Metadata[gwcommon.PolicyGK.String()] != nil {
		return p.printPolicy(node, w)
	}
	if node.Metadata != nil && node.Metadata[gwcommon.PolicyCRDGK.String()] != nil {
		return p.printPolicyCRD(node, w)
	}

	switch node.GKNN().GroupKind() {
	case gwcommon.GatewayGK:
		return p.printGateway(node, w)
	case gwcommon.GatewayClassGK:
		return p.printGatewayClass(node, w)
	case gwcommon.HTTPRouteGK:
		return p.printHTTPRoute(node, w)
	case gwcommon.NamespaceGK:
		return p.printNamespace(node, w)
	case gwcommon.ServiceGK:
		return p.printBackend(node, w)
	case common.DirectResponseGK:
		return p.printDirectResponse(node, w)
	default:
