...
func TestMain(m *testing.M) {
	...
	klog.InitFlags(fs)
	err := fs.Set("v", "3") // Set klog verbosity.
	if err != nil {
...
func testData(t *testing.T) map[schema.GroupKind][]*topology.Node {
	...
	directResponse1 := mustNewNode(t, &kgatewayv1.DirectResponse{
		TypeMeta: metav1.TypeMeta{
			Kind:       "DirectResponse",
			APIVersion: "gateway.kgateway.dev/v1alpha1",
		},
		ObjectMeta: metav1.ObjectMeta{
			Name:      "direct-response-1",
			Namespace: "ns-1",
		},
		Spec: kgatewayv1.DirectResponseSpec{
			Body:       ptr.To("Hello, world!"),
			StatusCode: 200,
		},
	})
