...
type Graph struct {
	Nodes   map[schema.GroupKind]map[types.NamespacedName]*gwctltopology.Node
	Sources []*gwctltopology.Node
...
func (g *Graph) AddNode(node *gwctltopology.Node) {
	klog.V(3).InfoS("AddNode", "node", node.GKNN())
	if g.Nodes == nil {
		g.Nodes = make(map[schema.GroupKind]map[types.NamespacedName]*gwctltopology.Node)
	}
	if g.Nodes[node.GKNN().GroupKind()] == nil {
		g.Nodes[node.GKNN().GroupKind()] = make(map[types.NamespacedName]*gwctltopology.Node)
	}
	g.Nodes[node.GKNN().GroupKind()][node.GKNN().NamespacedName()] = node
}

func (g *Graph) DeleteNode(node *gwctltopology.Node) {
...
func (g *Graph) AddEdge(from *gwctltopology.Node, to *gwctltopology.Node, relation *gwctltopology.Relation) {
	klog.V(3).InfoS("AddEdge", "from", from.GKNN(), "to", to.GKNN())
	if from.OutNeighbors == nil {
		from.OutNeighbors = make(map[*gwctltopology.Relation]map[common.GKNN]*gwctltopology.Node)
	}
	if from.OutNeighbors[relation] == nil {
		from.OutNeighbors[relation] = make(map[common.GKNN]*gwctltopology.Node)
	}
	from.OutNeighbors[relation][to.GKNN()] = to

	if to.InNeighbors == nil {
		to.InNeighbors = make(map[*gwctltopology.Relation]map[common.GKNN]*gwctltopology.Node)
	}
	if to.InNeighbors[relation] == nil {
		to.InNeighbors[relation] = make(map[common.GKNN]*gwctltopology.Node)
	}
	to.InNeighbors[relation][from.GKNN()] = from
}

func (g *Graph) RemoveEdge(from *gwctltopology.Node, to *gwctltopology.Node, relation *gwctltopology.Relation) {
...
func MustAccessObject[T any](node *gwctltopology.Node, concreteObj T) T {
...
func (b *Builder) UseRelationship(relation *gwctltopology.Relation) *Builder {
	b.Relations = append(b.Relations, relation)
	return b
}

func (b *Builder) UseRelationships(relations []*gwctltopology.Relation) *Builder {
...
func (b *Builder) Build() (*Graph, error) {
	...
	for _, obj := range b.Sources {
		node := &gwctltopology.Node{Object: obj}
	...
	for _, groupKind := range allGroupKinds {
		if groupKind == kgwcommon.LabelFauxGK {
			continue
		}
		...
		for _, resource := range resources {
			node := &gwctltopology.Node{Object: resource, Depth: inf}
	...
	for _, relation := range b.Relations {
		for _, fromNode := range graph.Nodes[relation.From] {
			for _, toNodeGKNN := range relation.NeighborFunc(fromNode.Object) {
				// Label selectors are not real resources in the cluster so they should not be added to the graph as nodes. So this needs to be done first.
				if strings.EqualFold(toNodeGKNN.GroupKind().String(), kgwcommon.LabelFauxGK.String()) {
					if _, ok := graph.Nodes[common.HTTPRouteGK]; !ok {
						continue
					}
					// Find all HTTPRoutes that match the selector criteria and add them as edges.
					// TODO - This would be handled better by changing the NeighborFunc to return a list of HTTPRoutes that match the selector criteria.
					//        To do this, it would need acccess to the graph to find the HTTPRoutes that match the selector criteria.
					//        Alternatively, we could generalize the pattern of label selectors.
					for _, node := range graph.Nodes[common.HTTPRouteGK] {
						...
						graph.AddEdge(fromNode, node, relation)
					}
					continue
				}
				...
				// For HTTPRoutes that delegate to other HTTPRoutes using a wildcard name, we need to find all HTTPRoutes that match the selector criteria.
	...
	q := []*gwctltopology.Node{} // q is a Queue used in the BFS.

	// Initialize the sources for the BFS
	for _, source := range b.Sources {
		gknn := (&gwctltopology.Node{Object: source}).GKNN()
	...
	for len(q) != 0 {
		...
		allNeighbors := []map[*gwctltopology.Relation]map[common.GKNN]*gwctltopology.Node{
			u.InNeighbors,
			u.OutNeighbors,
		}
