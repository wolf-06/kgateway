...
Modified to support Backends that are not type Service (e.g. DirectResponse)
*/
...
import (
	...
	parentrefgrantvalidator "sigs.k8s.io/gwctl/pkg/extension/refgrantvalidator"

	"github.com/kgateway-dev/kgateway/v2/pkg/kgwctl/common"
	"github.com/kgateway-dev/kgateway/v2/pkg/kgwctl/topology"
	topologygw "github.com/kgateway-dev/kgateway/v2/pkg/kgwctl/topology/kgateway"
)
...
func (a *Extension) discoverReferenceGrantsForBackends(graph *topology.Graph) error {
	...
	for _, gk := range []schema.GroupKind{gwcommon.ServiceGK, gwcommon.HTTPRouteGK, common.DirectResponseGK} {
		for _, backendNode := range graph.Nodes[gk] {
...
func (a *Extension) validateHTTPRoutes(graph *topology.Graph) error {
	for _, httpRouteNode := range graph.Nodes[gwcommon.HTTPRouteGK] {
		...
		for backendGKNN, backendNode := range topologygw.HTTPRouteNode(httpRouteNode).Backends() {
			...
			if httpRouteNode.GKNN().Namespace != backendGKNN.Namespace {
				...
				if !referenceAccepted {
					err := gwcommon.ReferenceNotPermittedError{ReferenceFromTo: gwcommon.ReferenceFromTo{
...
func (a *Extension) putReferenceGrantInNode(node *gwctltopology.Node, referenceGrant *gatewayv1beta1.ReferenceGrant) error {
	if node.Metadata == nil {
		node.Metadata = map[string]any{}
	}
	if node.Metadata[extensionName] == nil {
		node.Metadata[extensionName] = &parentrefgrantvalidator.NodeMetadata{
			ReferenceGrants: make(map[gwcommon.GKNN]*gatewayv1beta1.ReferenceGrant),
			Errors:          make([]error, 0),
		}
	}
	...
	gknn := gwcommon.GKNN{
		Group:     gwcommon.ReferenceGrantGK.Group,
		Kind:      gwcommon.ReferenceGrantGK.Kind,
...
func (a *Extension) putReferenceGrantErrorInNode(node *gwctltopology.Node, refGrantErr error) error {
	if node.Metadata == nil {
		node.Metadata = map[string]any{}
	}
	if node.Metadata[extensionName] == nil {
...
func Access(node *gwctltopology.Node) (*parentrefgrantvalidator.NodeMetadata, error) {
	...
	data, ok := rawData.(*parentrefgrantvalidator.NodeMetadata)
	if !ok {
...
func ReferenceGrantExposes(referenceGrant *gatewayv1beta1.ReferenceGrant, resource gwcommon.GKNN) bool {
...
func ReferenceGrantAccepts(referenceGrant *gatewayv1beta1.ReferenceGrant, resource gwcommon.GKNN) bool {
	resource.Name = ""
	for _, from := range referenceGrant.Spec.From {
		fromRef := gwcommon.GKNN{
...
func WithAdditionalResources(resources []*unstructured.Unstructured) referenceGrantFetcherOption { //nolint:revive
	return func(f *defaultReferenceGrantFetcher) {
		for _, resource := range resources {
			if resource.GroupVersionKind().GroupKind() == gwcommon.ReferenceGrantGK {
...
func NewDefaultReferenceGrantFetcher(factory gwcommon.Factory, options ...referenceGrantFetcherOption) *defaultReferenceGrantFetcher { //nolint:revive
...
func (f *defaultReferenceGrantFetcher) FetchReferenceGrantsForNamespace(namespace string) ([]*gatewayv1beta1.ReferenceGrant, error) {
	infos, err := f.factory.NewBuilder().
		...
		AllNamespaces(false).
		ResourceTypeOrNameArgs(true, []string{fmt.Sprintf("%v.%v", gwcommon.ReferenceGrantGK.Kind, gwcommon.ReferenceGrantGK.Group)}...).
		ContinueOnError().
		Do().
		Infos()
	if err != nil {
	...
	for _, u := range f.additionalResourcesByNamespace[namespace] {
		...
		if err := runtime.DefaultUnstructuredConverter.FromUnstructured(u.UnstructuredContent(), refGrant); err != nil {
			return nil, fmt.Errorf("converting local ReferenceGrant from Unstructurued to typed: %w", err)
		}
		result = append(result, refGrant)
	}
